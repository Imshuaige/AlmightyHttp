...
public class JsonPostRequest implements RequestBase {
    @Override
    public String request(String url, Map<String, String> param) {
        if (param!=null&&param.size()>0) {
            MultipartEntityBuilder builder = MultipartEntityBuilder.create();
            param.entrySet().stream().forEach(en->builder.addPart(en.getKey(),new StringBody(en.getValue(), ContentType.APPLICATION_JSON)));
